...
import { processAndGroupContactsByCEP, generateDomicileAddressKey } from './utils/domicileGroupUtils';
...
import { scanAndNotifyPriorityVisits } from './services/notificationService';
...
export default function App() {
          ...
          setContacts((prevContacts) => {
            ...
            data.contacts.forEach((remote: GoogleContact) => {
              ...
              if (existing) {
                ...
                contactMap.set(remote.id, remote);
              }
            });
  ...
  const handleUpdateEventStatus = async (eventId: string, newStatus: VisitStatus, observation?: string) => {
    ...
    if (targetEv) {
      logAuditEvent(
        'STATUS_VISITA_ALTERADO',
        'VISIT',
        `Visita "${targetEv.title}" atualizada para [${newStatus.toUpperCase()}] às ${timeStr}.`,
        observation ? `Obs: ${observation}` : undefined
      );
    }
  ...
  const handleRestoreData = (newDomiciles: Domicile[], newContacts: GoogleContact[], newEvents: CalendarEvent[]) => {
    ...
    logAuditEvent('DADOS_RESTAURADOS', 'SYSTEM', `Restauração executada: ${newContacts.length} pacientes, ${newDomiciles.length} domicílios, ${newEvents.length} visitas.`);
  };
